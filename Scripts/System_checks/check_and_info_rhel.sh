...
df -Ph | awk '{printf "%-35s %-8s %s\n",$6,$2,$5}'| while read dir rest; do echo -e $dir ' \t '$rest' \t '`ls -ld $dir| awk -F " " '{print $1, $3, $4}' `; done 
#вывод без указания процента свободного места
#df -Ph | awk '{printf "%-35s %s\n",$6,$2}'| while read dir rest; do echo -e $dir ' \t '$rest' \t '`ls -ld $dir| awk -F " " '{print $1, $3, $4}' `; done
echo
echo "---cat /etc/passwd  grep adm|ora---" 
cat /etc/passwd | grep adm 
cat /etc/passwd | grep ora 
echo "---cat /etc/group grep sapsys|dba|oper---" 
