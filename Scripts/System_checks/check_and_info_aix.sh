...
for i in nfsd ssh python Xorg rsync at ksh
do
echo -ne "    $i    " 
...
#par=`egrep "'192.168.124.134 prefer'|192.168.129.17" /etc/ntp.conf | wc -l`
...
df -g | awk '{printf "%-35s %-8s %s\n",$7,$2,$4}' | while read dir rest; do echo -e $dir ' \t '$rest' \t '`ls -ld $dir| awk -F " " '{print $1, $3, $4}' `; done 
...
echo
echo "---cat /etc/passwd | egrep 'adm|ora'---"
cat /etc/passwd | egrep 'adm|ora'
echo "---cat /etc/group grep sapsys|dba|oper|sapinst---"
cat /etc/group | grep sapsys
cat /etc/group | grep dba
cat /etc/group | grep oper
cat /etc/group | grep sapinst
